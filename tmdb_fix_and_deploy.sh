...
TOKEN_FILE="/opt/ai-movie-platform/.tmdb_v4.txt"
...
echo "Length: $LEN (>=100 expected)"
echo "Prefix: $(printf '%s' "$V4" | head -c 5)..."
echo "Dots:   $DOTS (must be 2)"
[[ "${V4:0:3}" != "eyJ" ]] && fail "Not a v4 JWT (must start with eyJ)"
[[ $LEN -lt 100 ]] && fail "Token too short (<100) — truncated or v3 key"
[[ $DOTS -ne 2 ]] && fail "Invalid JWT shape — exactly 2 dots required"

say "Live check: TMDb /3/configuration (must be 200)"
CODE=$(curl -s -o /dev/null -w "%{http_code}" \
  -H 'accept: application/json' \
  -H "Authorization: Bearer ${V4}" \
  https://api.themoviedb.org/3/configuration)
echo "HTTP: $CODE"
[[ "$CODE" != "200" ]] && fail "TMDb returned $CODE — wrong token / not approved / revoked"
